...
def setup_database(db_file, table_name):
    ...
    cursor.execute(f'''
        CREATE TABLE IF NOT EXISTS {table_name} (
            ...
            race TEXT,
            interest INTEGER,
            PRIMARY KEY (model_name, trial, major, pronouns, school, race,interest)
        )
    ''')
    conn.commit()
    conn.close()
...
class DatabaseTarget(luigi.Target):
    ...
    def exists(self):
        ...
        cursor.execute(
            ...
            WHERE model_name = ? AND trial = ? AND major = ? AND pronouns = ? AND school = ? AND race = ? AND interest = ?
            ''',
            (
                self.model_name,
                self.trial,
                self.condition['major'],
                self.condition['pronouns'],
                self.condition['school'],
                self.condition['race'],
                self.condition['interest']
...
class RunExperimentTask(luigi.Task):
    ...
    def run(self):
        ...
        data = (
            ...
            self.condition['major'], self.condition['pronouns'], self.condition['school'],self.condition['race'],self.condition['interest']
        )
        ...
        with DB_LOCK:
            ...
            INSERT OR IGNORE INTO {table_name} (
                ...
                major, pronouns, school, race, interest
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            '''
